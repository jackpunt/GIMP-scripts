...
(define (card-to-template image drawable undo context)
  ...
  ;; assert LAST-ILXY is valid [either First or Use-Template]
  (let ((ilxy (next-ilxy-empty)))                  ; possibly a new image-template?
    (and undo (gimp-image-undo-group-start image)) ; mark for undo
    (card-to-this-ilxy image ilxy context)
    (set-ilxy ilxy)
...
(define (get-entrypoints) 
  ...
  (define (xoff) (nth 2 TEMPLATE-MIN-OFFSET-INC)) ; (xoff,yoff) where color should be
  (define (yoff) (nth 3 TEMPLATE-MIN-OFFSET-INC)) ; in original card coordinates
  (define (xinc) (nth 4 TEMPLATE-MIN-OFFSET-INC)) ; slightly greater than cardw
  (define (yinc) (nth 5 TEMPLATE-MIN-OFFSET-INC)) ; slightly greater than cardh


  (define (is-card-at0 image layer lx ly)
    ;; look at spot (xoff, yoff) of card relative to location (lx, ly) of template.
    ;; from (lastx lasty) in template image; look at (xoff,yoff)
    ;;(message-string "is-card-at:" image layer lx ly)
    ;; rotate xoff, yoff [Clockwise] 
    ;; find card(xoff,yoff) in template image:
    (let* ((srcw (car (gimp-image-width image)))
	   ...
	   (x (+ lx (if norot? (xoff) (- temph (yoff)))))
	   (y (+ ly (if norot? (yoff) (xoff))))
	   (color (car (gimp-image-pick-color image layer x y FALSE TRUE 5)))
	   (dist (color-dist color TEMPLATE-EMPTY))
	   )
      ;;(message-string "(x,y)" (list x y) "color" color "dist" dist)
      (> dist 30)))
  ...
  (define (is-card-at image layer lx ly)
    ...
    (catch #t (> (get-alpha image layer (+ lx (xoff)) (+ ly (yoff))) 0)))
  ...
  (define (next-image-layer-xy image0 layer0 . file)
    ;;(message-string "next-image-layer-xy:" image0 layer0 file "open-ilxy =" open-ilxy )
	...
	(let* ((filename (if (pair? file)
	  ...
	  ;;(message-string "next-image-layer-xy" filename ", ilxy:" ilxy "LAST-ILXY" LAST-ILXY)
	  ilxy
	  )))
